...
import { accounts, sessions, users, verificationTokens } from "~/drizzle/schemas/auth";
...
export function createDrizzleAdapter(db: LibSQLDatabase): Adapter {
    return {
        ...
        async getUser(id) {
            const rows = await db.select().from(users).where(eq(users.id, id)).limit(1).all();
            const row = rows[0];
            return row ?? null;
        },
        async getUserByEmail(email) {
            const rows = await db.select().from(users).where(eq(users.email, email)).limit(1).all();
            const row = rows[0];
            return row ?? null;
        },
        ...
        async updateUser({ id, ...userData }) {
            ...
            const row = await db.update(users).set(userData).where(eq(users.id, id)).returning().get();
            return row;
        },
        ...
        async updateSession(session) {
            ...
            }).where(eq(sessions.sessionToken, session.sessionToken)).returning().get()
            return {
                ...row,
                expires: new Date(row.expires)
            }
        ...
        async deleteSession(sessionToken) {
            await db.delete(sessions).where(eq(sessions.sessionToken, sessionToken)).returning().get()
        },
        ...
        async useVerificationToken({ identifier, token }) {
            ...
            const rows = await db.select().from(verificationTokens).where(eq(verificationTokens.token, token)).limit(1).all();
            const row = rows[0];
            if (!row) return null;
            // Then delete it.
            await db
                .delete(verificationTokens)
                .where(and(eq(verificationTokens.token, token), eq(verificationTokens.identifier, identifier))).returning().get()
            // Then return it.
            return {
                ...row,
                expires: new Date(row.expires)
